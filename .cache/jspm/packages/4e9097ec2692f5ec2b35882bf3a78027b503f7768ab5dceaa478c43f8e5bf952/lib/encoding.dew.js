import { dew as _encodingIndexesDewDew } from "./encoding-indexes.dew.js";
var exports = {},
    _dewExec = false;
...
export function dew() {
  ...
  (function (global) {
    ...
    if (exports && !global["encoding-indexes"]) {
      global["encoding-indexes"] = _encodingIndexesDewDew()["encoding-indexes"];
    } //
    // Utilities
    //
    ...
    function Stream(tokens) {
      /** @type {!Array.<number>} */
      (this || _global).tokens = [].slice.call(tokens); // Reversed as push/pop is more efficient than shift/unshift.

      (this || _global).tokens.reverse();
    }
    ...
    Stream.prototype = {
      ...
      endOfStream: function () {
        return !(this || _global).tokens.length;
      },
       ...
       */
      read: function () {
        if (!(this || _global).tokens.length) return end_of_stream;
        return (this || _global).tokens.pop();
      },
      ...
      prepend: function (token) {
        if (Array.isArray(token)) {
          ...
          while (tokens.length) (this || _global).tokens.push(tokens.pop());
        } else {
          (this || _global).tokens.push(token);
        }
      },
      ...
      push: function (token) {
        if (Array.isArray(token)) {
          ...
          while (tokens.length) (this || _global).tokens.unshift(tokens.shift());
        } else {
          (this || _global).tokens.unshift(token);
        }
      }
    }; //
    // 5. Encodings
    ...
    function TextDecoder(label, options) {
      // Web IDL conventions
      if (!((this || _global) instanceof TextDecoder)) throw TypeError('Called as a function. Did you forget \'new\'?');
      ...
      (this || _global)._ignoreBOM = false;
      /** @private @type {boolean} */

      (this || _global)._BOMseen = false;
      /** @private @type {string} */

      (this || _global)._error_mode = 'replacement';
      /** @private @type {boolean} */

      (this || _global)._do_not_flush = false; // 1. Let encoding be the result of getting an encoding from
      // label.
      ...
      var dec = this || _global; // 4. Set dec's encoding to encoding.
      ...
      if (!Object.defineProperty) {
        (this || _global).encoding = dec._encoding.name.toLowerCase();
        (this || _global).fatal = dec._error_mode === 'fatal';
        (this || _global).ignoreBOM = dec._ignoreBOM;
      } // 7. Return dec.
    ...
    if (Object.defineProperty) {
      ...
      Object.defineProperty(TextDecoder.prototype, 'encoding', {
        /** @this {TextDecoder} */
        get: function () {
          return (this || _global)._encoding.name.toLowerCase();
        }
      ...
      Object.defineProperty(TextDecoder.prototype, 'fatal', {
        /** @this {TextDecoder} */
        get: function () {
          return (this || _global)._error_mode === 'fatal';
        }
      ...
      Object.defineProperty(TextDecoder.prototype, 'ignoreBOM', {
        /** @this {TextDecoder} */
        get: function () {
          return (this || _global)._ignoreBOM;
        }
      });
    }
    /**
    ...
    TextDecoder.prototype.decode = function decode(input, options) {
      ...
      if (!(this || _global)._do_not_flush) {
        (this || _global)._decoder = decoders[(this || _global)._encoding.name]({
          fatal: (this || _global)._error_mode === 'fatal'
        });
        (this || _global)._BOMseen = false;
      ...
      (this || _global)._do_not_flush = Boolean(options['stream']); // 3. If input is given, push a copy of input to stream.
      ...
      while (true) {
        ...
        result = (this || _global)._decoder.handler(input_stream, token); // 2. If result is finished, return output, serialized.
      ...
      if (!(this || _global)._do_not_flush) {
        do {
          result = (this || _global)._decoder.handler(input_stream, input_stream.read());
          if (result === finished) break;
      ...
      function serializeStream(stream) {
        ...
        if (includes(['UTF-8', 'UTF-16LE', 'UTF-16BE'], (this || _global)._encoding.name) && !(this || _global)._ignoreBOM && !(this || _global)._BOMseen) {
          if (stream.length > 0 && stream[0] === 0xFEFF) {
            // 1. If token is U+FEFF, set BOM seen flag.
            (this || _global)._BOMseen = true;
            stream.shift();
      ...
      return serializeStream.call(this || _global, output);
    ...
    function TextEncoder(label, options) {
      // Web IDL conventions
      if (!((this || _global) instanceof TextEncoder)) throw TypeError('Called as a function. Did you forget \'new\'?');
      ...
      (this || _global)._encoder = null; // Non-standard

      /** @private @type {boolean} */

      (this || _global)._do_not_flush = false;
      /** @private @type {string} */

      (this || _global)._fatal = Boolean(options['fatal']) ? 'fatal' : 'replacement'; // 1. Let enc be a new TextEncoder object.

      var enc = this || _global; // 2. Set enc's encoding to UTF-8's encoder.
      ...
      if (!Object.defineProperty) (this || _global).encoding = enc._encoding.name.toLowerCase(); // 3. Return enc.
    ...
    TextEncoder.prototype.encode = function encode(opt_string, options) {
      ...
      if (!(this || _global)._do_not_flush) (this || _global)._encoder = encoders[(this || _global)._encoding.name]({
        fatal: (this || _global)._fatal === 'fatal'
      });
      (this || _global)._do_not_flush = Boolean(options['stream']); // 1. Convert input to a stream.
      ...
      while (true) {
        ...
        result = (this || _global)._encoder.handler(input, token);
        if (result === finished) break;
        if (Array.isArray(result)) output.push.apply(output,
        /**@type {!Array.<number>}*/
        result);else output.push(result);
      ...
      if (!(this || _global)._do_not_flush) {
        while (true) {
          result = (this || _global)._encoder.handler(input, input.read());
          if (result === finished) break;
          if (Array.isArray(result)) output.push.apply(output,
          /**@type {!Array.<number>}*/
          result);else output.push(result);
    ...
    function UTF8Decoder(options) {
      ...
      (this || _global).handler = function (stream, bite) {
    ...
    function UTF8Encoder(options) {
      ...
      (this || _global).handler = function (stream, code_point) {
