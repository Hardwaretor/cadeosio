import _fs from "fs";
import { dew as _polyfillsDewDew } from "./polyfills.dew.js";
import { dew as _legacyStreamsDewDew } from "./legacy-streams.dew.js";
...
export function dew() {
  ...
  var polyfills = _polyfillsDewDew();

  var legacy = _legacyStreamsDewDew();

  var clone = _cloneDewDew();
  ...
  if (!fs[gracefulQueue]) {
    ...
    var queue = _global[gracefulQueue] || [];
    ...
    if (/\bgfs4\b/i.test(process.env.NODE_DEBUG || '')) {
      process.on('exit', function () {
        ...
        _assert.equal(fs[gracefulQueue].length, 0);
      });
    }
  }

  if (!_global[gracefulQueue]) {
    publishQueue(_global, fs[gracefulQueue]);
  }
