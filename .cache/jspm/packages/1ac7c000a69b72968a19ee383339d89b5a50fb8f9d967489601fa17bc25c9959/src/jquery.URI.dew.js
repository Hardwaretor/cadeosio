...
export function dew() {
  ...
  (function (root, factory) {
    ...
    if (exports) {
      // Node
      exports = factory(_nodeRequire('jquery'), _URIDewDew());
    ...
    $.attrHooks.uri.get = _attrHooks.get; // general URI accessor
    ...
    URI.prototype.build = function (deferBuild) {
      if ((this || _global)._dom_element) {
        ...
        (this || _global)._dom_element.setAttribute((this || _global)._dom_attribute, (this || _global)._string);

        (this || _global)._dom_element[(this || _global)._dom_attribute] = (this || _global)._string;
    ...
    function uriPseudo(elem, text) {
      var match, property, uri; // skip anything without src|href|action and bad :uri() syntax
    ...
    $.expr[':'].uri = uriSizzle; // extending existing object rather than defining something new,
