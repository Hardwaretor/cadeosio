...
import { dew as _indexDewDew } from "textextensions/index.dew.js";
import { dew as _indexDew2Dew } from "binaryextensions/index.dew.js";
var exports = {},
    _dewExec = false;
export function dew() {
  ...
  _dewExec = true;
  // Import
  const pathUtil = _path;

  const textExtensions = _indexDewDew();

  const binaryExtensions = _indexDew2Dew();
  /**
   * Is Text (Synchronous)
  ...
  function isTextSync(filename, buffer) {
    // Prepare
    let isText = null; // Test extensions

    if (filename) {
      // Extract filename
      const parts = pathUtil.basename(filename).split('.').reverse(); // Cycle extensions
  ...
  function getEncodingSync(buffer, opts) {
    ...
    const binaryEncoding = 'binary'; // Discover

    if (opts == null) {
      // Start
      const chunkLength = 24;
      let encoding = getEncodingSync(buffer, {
        chunkLength
      });
      ...
      let encoding = textEncoding; // Detect encoding
