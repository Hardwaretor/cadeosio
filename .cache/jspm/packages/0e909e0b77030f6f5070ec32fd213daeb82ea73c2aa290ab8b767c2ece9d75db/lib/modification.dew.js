...
export function dew() {
  ...
  const cloneObject = _indexDewDew();

  const predicates_1 = _predicatesDewDew();

  const walking_1 = _walkingDewDew();
  ...
  function removeFakeRootElements(ast) {
    const injectedNodes = walking_1.queryAll(ast, predicates_1.predicates.AND(node => !node.__location, predicates_1.predicates.hasMatchingTagName(/^(html|head|body)$/i)), undefined, // Don't descend past 3 levels 'document > html > head|body'
