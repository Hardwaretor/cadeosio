  ...
  (function (OPT) {
    ...
    (function (def) {
      ...
      function reset_def(tw, compressor, def) {
        ...
        def.escaped = false;
        def.fixed = !def.scope.pinned() && !compressor.exposed(def) && !(def.init instanceof AST_Function && def.init !== def.scope) && def.init;
      ...
      def(AST_SymbolRef, function (tw, descend, compressor) {
        ...
        if (d.fixed === undefined || !safe_to_read(tw, d)) {
          ...
          if (value instanceof AST_Lambda && recursive_ref(tw, d)) {
            ...
            d.single_use = value instanceof AST_Lambda && !value.pinned() || d.scope === this.scope && value.is_constant_expression();
          } else {
            d.single_use = false;
          }
    ...
    AST_Toplevel.DEFMETHOD("reset_opt_flags", function (compressor) {
      ...
      tw.defun_visited = Object.create(null); // Record the loop body in which `AST_SymbolDeclaration` is first encountered

      tw.in_loop = null;
      tw.loop_ids = Object.create(null); // Stack of look-up tables to keep track of whether a `SymbolDef` has been
    ...
    function tighten_body(statements, compressor) {
      ...
      function collapse(statements, compressor) {
        ...
        var scanner = new TreeTransformer(function (node) {
          if (abort) return node; // Skip nodes before `candidate` as quickly as possible
        ...
        while (--stat_index >= 0) {
          ...
          while (candidates.length > 0) {
            ...
            if (!scan_lhs && !scan_rhs) continue; // Locate symbols which may execute code outside of scanning range
        ...
        function extract_args() {
          ...
          if (fn instanceof AST_Function && !fn.name && !fn.uses_arguments && !fn.pinned() && (iife = compressor.parent()) instanceof AST_Call && iife.expression === fn) {
    ...
    (function (def) {
      ...
      var fn = makePredicate(["charCodeAt", "getDate", "getDay", "getFullYear", "getHours", "getMilliseconds", "getMinutes", "getMonth", "getSeconds", "getTime", "getTimezoneOffset", "getUTCDate", "getUTCDay", "getUTCFullYear", "getUTCHours", "getUTCMilliseconds", "getUTCMinutes", "getUTCMonth", "getUTCSeconds", "getYear", "indexOf", "lastIndexOf", "localeCompare", "push", "search", "setDate", "setFullYear", "setHours", "setMilliseconds", "setMinutes", "setMonth", "setSeconds", "setTime", "setUTCDate", "setUTCFullYear", "setUTCHours", "setUTCMilliseconds", "setUTCMinutes", "setUTCMonth", "setUTCSeconds", "setYear", "toExponential", "toFixed", "toPrecision"]);
      def(AST_Call, function (compressor) {
        if (!compressor.option("unsafe")) return false;
        var exp = this.expression;
    ...
    convert_to_predicate(static_fns); // methods to evaluate a constant expression
    ...
    AST_Scope.DEFMETHOD("drop_unused", function (compressor) {
      ...
      var tt = new TreeTransformer(function (node, descend, in_list) {
        ...
        if (node instanceof AST_Definitions && !(parent instanceof AST_ForIn && parent.init === node)) {
          ...
          var body = [],
              head = [],
              tail = []; // for unused names whose initialization has
    ...
    OPT(AST_Call, function (self, compressor) {
      ...
      if (compressor.option("unused") && is_func && !fn.uses_arguments && !fn.pinned()) {
        var pos = 0,
            last = 0;
        ...
        if (is_undeclared_ref(exp)) switch (exp.name) {
          ...
          case "call":
            ...
            if (func instanceof AST_Lambda && !func.contains_this()) {
              return (self.args.length ? make_sequence(this, [self.args[0], make_node(AST_Call, self, {
                expression: exp.expression,
      ...
      if (is_func) {
        ...
        if (can_inline && !fn.uses_arguments && !fn.pinned() && !(fn.name && fn instanceof AST_Function) && (value = can_flatten_body(stat)) && (exp === fn || compressor.option("unused") && (def = exp.definition()).references.length == 1 && !recursive_ref(compressor, def) && fn.is_constant_expression(exp.scope)) && !self.pure && !fn.contains_this() && can_inject_symbols()) {
          fn._squeezed = true;
    ...
    OPT(AST_Binary, function (self, compressor) {
      ...
      if (compressor.option("comparisons")) switch (self.operator) {
        ...
        case "!==":
          ...
          if (self.left.is_string(compressor) && self.right.is_string(compressor) || self.left.is_number(compressor) && self.right.is_number(compressor) || self.left.is_boolean(compressor) && self.right.is_boolean(compressor) || self.left.equivalent_to(self.right)) {
    ...
    OPT(AST_SymbolRef, function (self, compressor) {
      if (!compressor.option("ie8") && is_undeclared_ref(self) // testing against `self.scope.uses_with` is an optimization
      ...
      if (compressor.option("reduce_vars") && is_lhs(self, parent) !== self) {
        ...
        if (single_use && fixed instanceof AST_Lambda) {
          if (def.scope !== self.scope && (!compressor.option("reduce_funcs") || def.escaped == 1 || fixed.inlined)) {
            single_use = false;
    ...
    OPT(AST_Conditional, function (self, compressor) {
      if (!compressor.option("conditionals")) return self; // This looks like lift_sequences(), should probably be under "sequences"
      ...
      if (seq_tail instanceof AST_Assign) {
        ...
        if ((is_eq || consequent === seq_tail) && alt_tail instanceof AST_Assign && seq_tail.operator == alt_tail.operator && seq_tail.left.equivalent_to(alt_tail.left) && (is_eq && !seq_tail.left.has_side_effects(compressor) || !condition.has_side_effects(compressor) && can_shift_lhs_of_tail(consequent) && can_shift_lhs_of_tail(alternative))) {
      ...
      if (consequent instanceof AST_Call && alternative.TYPE === consequent.TYPE && consequent.args.length > 0 && consequent.args.length == alternative.args.length && consequent.expression.equivalent_to(alternative.expression) && !condition.has_side_effects(compressor) && !consequent.expression.has_side_effects(compressor) && typeof (arg_index = single_arg_diff()) == "number") {
      ...
      function booleanize(node) {
        if (node.is_boolean(compressor)) return node; // !!expression
      ...
      function can_shift_lhs_of_tail(node) {
        ...
        for (var i = exprs.length - 1; --i >= 0;) {
          var expr = exprs[i];
          if (!(expr instanceof AST_Assign) && expr.has_side_effects(compressor) || expr.operator != "=" || expr.left.has_side_effects(compressor) || expr.right.has_side_effects(compressor)) return false;
        }
    ...
    function safe_to_flatten(value, compressor) {
      ...
      if (!value) return false;
      return !(value instanceof AST_Lambda) || compressor.parent() instanceof AST_New || !value.contains_this();
    }
    ...
    OPT(AST_Sub, function (self, compressor) {
      ...
      if (compressor.option("arguments") && expr instanceof AST_SymbolRef && expr.name == "arguments" && expr.definition().orig.length == 1 && prop instanceof AST_Number && (fn = expr.scope) === compressor.find_parent(AST_Lambda)) {
        ...
        if (argname && compressor.has_directive("use strict")) {
          ...
          if (!compressor.option("reduce_vars") || expr.definition().reassigned || def.assignments || def.orig.length > 1) {
            argname = null;
          }
