import { dew as _indexDewDew } from "launchpad/index.dew.js";
  ...
  function expand(names, browserOptions) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (unsupported.length > 0) {
        throw new Error(`The following browsers are unsupported: ${unsupported.join(', ')}. ` + `(All supported browsers: ${exports.supported().join(', ')})`);
      }

      const installedByName = yield exports.detect(browserOptions);
      const installed = Object.keys(installedByName); // Opting to use everything?
      ...
      if (missing.length > 0) {
        throw new Error(`The following browsers were not found: ${missing.join(', ')}. ` + `(All installed browsers found: ${installed.join(', ')})`);
      }
