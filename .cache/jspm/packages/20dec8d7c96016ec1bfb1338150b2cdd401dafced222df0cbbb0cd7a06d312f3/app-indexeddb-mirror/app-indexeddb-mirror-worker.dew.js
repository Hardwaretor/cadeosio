...
export function dew() {
  ...
  (function () {
    ...
    function AppIndexedDBMirrorWorker(_dbName, _storeName) {
      ...
      (this || _global)[DB_NAME] = _dbName;
      (this || _global)[STORE_NAME] = _storeName; // Maybe useful in case we want to notify clients of changes..

      (this || _global)[CLIENT_PORTS] = new Array();
      (this || _global)[DB_OPENS] = null;
      (this || _global).supportsIndexedDB = self.indexedDB != null;

      if (!self.Promise || !(this || _global).supportsIndexedDB) {
    ...
    AppIndexedDBMirrorWorker.prototype = {
      openDb: function () {
        (this || _global).__dbOpens = (this || _global).__dbOpens || new Promise(function (resolve, reject) {
          console.log('Opening database..');
          var request = self.indexedDB.open((this || _global)[DB_NAME], DB_VERSION);

          request.onupgradeneeded = function (event) {
            console.log('Upgrade needed:', event.oldVersion, '=>', event.newVersion);
            var context = {
              database: request.result,
              storeName: (this || _global)[STORE_NAME],
              dbName: (this || _global)[DB_NAME]
            };

            for (var i = event.oldVersion; i < event.newVersion; ++i) {
              MIGRATIONS[i] && MIGRATIONS[i].call(this || _global, context);
            }
          }.bind(this || _global);
        ...
        return (this || _global).__dbOpens;
      },
      closeDb: function () {
        if ((this || _global).__dbOpens == null) {
          return Promise.resolve();
        }

        return this.openDb().then(function (db) {
          (this || _global).__dbOpens = null;
      ...
      transaction: function (method, key, value) {
        ...
        switch (method) {
          case 'get':
            return this.get((this || _global)[STORE_NAME], key);

          case 'set':
            return this.set((this || _global)[STORE_NAME], key, value);
        }
      ...
      validateSession: function (session) {
        return Promise.all([this.openDb(), this.get(INTERNAL_STORE_NAME, 'session')]).then(function (results) {
          var db = results[0];
          ...
          if (session !== currentSession) {
            if (currentSession != null) {
              operations.push(this.clear((this || _global)[STORE_NAME]));
            }
      ...
      registerClient: function (port) {
        ...
        var isPortInClient = (port.toString() in (this || _global)[CLIENT_PORTS]);

        if (!isPortInClient) {
          (this || _global)[CLIENT_PORTS].push(port);
        }

        port.start();
        port.postMessage({
          'type': 'app-mirror-connected',
          'supportsIndexedDB': (this || _global).supportsIndexedDB
        });
      ...
      handleClientMessage: function (event, port) {
        ...
        switch (event.data['type']) {
          ...
          case 'app-mirror-transaction':
            this.transaction(event.data['method'], event.data['key'], event.data['value']).then(function (result) {
              port.postMessage({
          ...
          case 'app-mirror-disconnect':
            var index = (this || _global)[CLIENT_PORTS].indexOf(port);

            if (index !== -1) {
              (this || _global)[CLIENT_PORTS].splice(index, 1);
            }
