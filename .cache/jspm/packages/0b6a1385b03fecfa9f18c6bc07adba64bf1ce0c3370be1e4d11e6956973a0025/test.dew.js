...
export function dew() {
  ...
  var test = _nodeRequire('tape').test;

  var from = _nodeRequire('from2');

  var crypto = _crypto;

  var sink = _nodeRequire('flush-write-stream');
  ...
  test('waits till all are flowing', function (t) {
    t.plan(1);
    var source = from(['wa', 'dup']);
    var instance = cloneable(source); // we create a clone
  ...
  test('big file', function (t) {
    t.plan(13);
    var stream = cloneable(fs.createReadStream(path.join(import.meta.url.startsWith('file:') ? decodeURI(import.meta.url.slice(0, import.meta.url.lastIndexOf('/')).slice(7 + (typeof process !== 'undefined' && process.platform === 'win32'))) : new URL(import.meta.url.slice(0, import.meta.url.lastIndexOf('/'))).pathname, 'big')));
    var hash = crypto.createHash('sha1');
    hash.setEncoding('hex');
    var toCheck;
    fs.createReadStream(path.join(import.meta.url.startsWith('file:') ? decodeURI(import.meta.url.slice(0, import.meta.url.lastIndexOf('/')).slice(7 + (typeof process !== 'undefined' && process.platform === 'win32'))) : new URL(import.meta.url.slice(0, import.meta.url.lastIndexOf('/'))).pathname, 'big')).pipe(hash).once('readable', function () {
      toCheck = hash.read();
      t.ok(toCheck);
    });
    ...
    function pipe(s, num) {
      ...
      var dest = path.join(import.meta.url.startsWith('file:') ? decodeURI(import.meta.url.slice(0, import.meta.url.lastIndexOf('/')).slice(7 + (typeof process !== 'undefined' && process.platform === 'win32'))) : new URL(import.meta.url.slice(0, import.meta.url.lastIndexOf('/'))).pathname, 'out');
      s.pipe(fs.createWriteStream(dest)).on('finish', function () {
        t.pass('finish for ' + num);
        var destHash = crypto.createHash('sha1');
        destHash.setEncoding('hex');
        fs.createReadStream(dest).pipe(destHash).once('readable', function () {
    ...
    setImmediate(pipe.bind(null, stream, 1)); // Pipe in the same event loop tick

    pipe(stream.clone(), 0); // Pipe a long time after
