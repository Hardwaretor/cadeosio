import { dew as _indexDewDew } from "assert-plus/index.dew.js";
...
export function dew() {
  ...
  function Parser(config) {
    ...
    var self = this || _global; // Allow interspersed arguments (true by default).

    (this || _global).interspersed = config.interspersed !== undefined ? config.interspersed : true; // Don't allow unknown flags (true by default).

    (this || _global).allowUnknown = config.allowUnknown !== undefined ? config.allowUnknown : false;
    (this || _global).options = config.options.map(function (o) {
      return shallowCopy(o);
    });
    (this || _global).optionFromName = {};
    (this || _global).optionFromEnv = {};

    for (var i = 0; i < (this || _global).options.length; i++) {
      var o = (this || _global).options[i];
  ...
  Parser.prototype.parse = function parse(inputs) {
    var self = this || _global; // Old API was `parse([argv, [slice]])`
    ...
    outer: while (i < args.length) {
      ...
      if (arg === '--') {
        ...
        var option = (this || _global).optionFromName[name];

        if (!option) {
          if (!(this || _global).allowUnknown) throw new Error(format('unknown option: "--%s"', name));else if ((this || _global).interspersed) _args.push(arg);else break outer;
        } else {
        ...
        while (j < arg.length) {
          ...
          if (!option) {
            ...
            if ((this || _global).allowUnknown) {
              if ((this || _global).interspersed) {
                _args.push(arg);
      ...
      } else if ((this || _global).interspersed) {
    ...
    Object.keys((this || _global).optionFromEnv).forEach(function (envname) {
      var val = env[envname];
    ...
    (this || _global).options.forEach(function (o) {
  ...
  Parser.prototype.bashCompletion = function bashCompletion(args) {
    ...
    return bashCompletionFromOptions({
      ...
      options: (this || _global).options
    });
  }; // ---- Bash completion


  const BASH_COMPLETION_TEMPLATE_PATH = path.join(import.meta.url.startsWith('file:') ? decodeURI(import.meta.url.slice(0, import.meta.url.lastIndexOf('/')).slice(7 + (typeof process !== 'undefined' && process.platform === 'win32'))) : new URL(import.meta.url.slice(0, import.meta.url.lastIndexOf('/'))).pathname, '../etc/dashdash.bash_completion.in');
  /**
