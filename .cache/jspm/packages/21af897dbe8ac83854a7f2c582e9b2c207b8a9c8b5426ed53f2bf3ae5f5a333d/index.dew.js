...
export function dew() {
  ...
  /^(?=[^^])/, function startingReplacer() {
    return !/\/(?!$)/.test(this || _global) // > If the pattern does not contain a slash /,
  ...
  class IgnoreBase {
    constructor({
      ...
      (this || _global)._rules = [];
      (this || _global)._ignorecase = ignorecase;
      define(this || _global, KEY_IGNORE, true);

      this._initCache();
    }

    _initCache() {
      (this || _global)._cache = Object.create(null);
    } // @param {Array.<string>|string|Ignore} pattern


    add(pattern) {
      (this || _global)._added = false;

      if (typeof pattern === 'string') {
        pattern = pattern.split(/\r?\n/g);
      }

      make_array(pattern).forEach((this || _global)._addPattern, this || _global); // Some rules have just added to the ignore,
      // making the behavior changed.

      if ((this || _global)._added) {
        this._initCache();
      }
    ...
    _addPattern(pattern) {
      ...
      if (pattern && pattern[KEY_IGNORE]) {
        (this || _global)._rules = (this || _global)._rules.concat(pattern._rules);
        (this || _global)._added = true;
        return;
      }

      if (checkPattern(pattern)) {
        const rule = createRule(pattern, (this || _global)._ignorecase);
        (this || _global)._added = true;

        (this || _global)._rules.push(rule);
      }
    }
    ...
    _filter(path, slices) {
      ...
      if (path in (this || _global)._cache) {
        return (this || _global)._cache[path];
      }
      ...
      slices.pop();
      return (this || _global)._cache[path] = slices.length // > It is not possible to re-include a file if a parent directory of
    ...
    _test(path) {
      ...
      (this || _global)._rules.forEach(rule => {
  ...
  typeof process !== 'undefined' && (process.env && process.env.IGNORE_TEST_WIN32 || process.platform === 'win32')) {
    ...
    IgnoreBase.prototype._filter = function filterWin32(path, slices) {
      ...
      return filter.call(this || _global, path, slices);
    };
  }

  exports = options => new IgnoreBase(options);
