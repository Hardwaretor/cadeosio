...
import { dew as _indexDew4Dew } from "convert-source-map/index.dew.js";
import { dew as _indexDew5Dew } from "strip-bom/index.dew.js";
...
export function dew() {
  ...
  var convert = _indexDew4Dew();

  var stripBom = _indexDew5Dew();
  ...
  exports.init = function init(options) {
    function sourceMapInit(file, encoding, callback) {
      ...
      if (options && options.loadMaps) {
        ...
        if (sourceMap) {
          sourceMap = sourceMap.toObject(); // sources in map are relative to the source file
          ...
          if (mapComment) {
            ...
            fileContent = convert.removeMapFileComments(fileContent); // if no comment try map file with same name as source file
          } else {
          ...
          sourceMap.sources.forEach(function (source, i) {
            ...
            if (!sourceMap.sourcesContent[i]) {
              ...
              if (absPath === file.path) {
                sourceContent = fileContent; // else load content from file
              } else {
                try {
  ...
  exports.write = function write(destPath, options) {
    ...
    function sourceMapWrite(file, encoding, callback) {
      ...
      var sourceMap = file.sourceMap; // fix paths if Windows style paths
      ...
      if (options.includeContent) {
        sourceMap.sourcesContent = sourceMap.sourcesContent || []; // load missing source content
      ...
      if (!destPath) {
        ...
        var sourceMapPath = path.join(file.base, destPath, file.relative) + '.map'; // add new source map file to stream
