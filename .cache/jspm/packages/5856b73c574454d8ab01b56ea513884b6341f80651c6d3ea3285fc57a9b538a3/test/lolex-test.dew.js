import { dew as _lolexSrcDewDew } from "../src/lolex-src.dew.js";
...
export function dew() {
  ...
  {
    var referee = _nodeRequire("referee");

    var lolex = _lolexSrcDewDew();

    var sinon = _nodeRequire("sinon");

    _global.lolex = lolex; // For testing eval
  }
  ...
  describe("issue #59", function () {
    ...
    it("should install and uninstall the clock on a custom target", function () {
      clock = lolex.install(context); // this would throw an error before issue #59 was fixed
  ...
  describe("lolex", function () {
    ...
    describe("tick", function () {
      ...
      it("triggers multiple simultaneous timers with zero callAt", function () {
        ...
        }), sinon.spy(), sinon.spy()]; // First spy calls another setTimeout with delay=0
      ...
      it("calls function with global object or null (strict mode) as this", function () {
        ...
        assert(stub.calledOn(_global) || stub.calledOn(null));
      });
    ...
    describe("runAll", function () {
      ...
      it("new timers added while running are also run", function () {
        ...
        }), sinon.spy()]; // Spy calls another setTimeout
    ...
    describe("runToLast", function () {
      ...
      it("new timers added with a call time ealier than the last existing timer are run", function () {
        this.clock = lolex.createClock();
        var test = this;
        var spies = [sinon.spy(), sinon.spy(function () {
          test.clock.setTimeout(spies[2], 50);
        }), sinon.spy()];
        this.clock.setTimeout(spies[0], 100); // Spy calls another setTimeout
    ...
    describe("date", function () {
      beforeEach(function () {
        ...
        this.Date = _global.Date;
      });
      afterEach(function () {
        _global.Date = this.Date;
      });
    ...
    describe("stubTimers", function () {
      ...
      afterEach(function () {
        ...
        if (this.dateNow === undefined) {
          delete _global.Date.now;
        } else {
          _global.Date.now = this.dateNow;
        }
      });
      ...
      if (Object.getPrototypeOf(_global)) {
        delete _global.hasOwnPropertyTest;

        Object.getPrototypeOf(_global).hasOwnPropertyTest = function () {};

        if (!_global.hasOwnProperty("hasOwnPropertyTest")) {
          it("deletes global property on uninstall if it was inherited onto the global object", function () {
            ...
            Object.getPrototypeOf(_global).tick = function () {};

            this.clock = lolex.install(0, ["tick"]);
            assert.isTrue(_global.hasOwnProperty("tick"));
            this.clock.uninstall();
            assert.isFalse(_global.hasOwnProperty("tick"));
            delete Object.getPrototypeOf(_global).tick;
          });
        }

        delete Object.getPrototypeOf(_global).hasOwnPropertyTest;
      }
