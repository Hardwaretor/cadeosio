...
export function dew() {
  ...
  var maybe = _indexDewDew();

  var assert = _assert;

  var Promise = _global.Promise || _nodeRequire("promise");
  ...
  describe("maybe", function () {
    ...
    it("should allow errors thrown in the callback to be uncaught", function (done) {
      var mochaHandler; // Temporarily remove Mocha's global error handling so we can
      // verify error is indeed uncaught by installing our own
      // global error handler.

      if (process.browser) {
        mochaHandler = _global.onerror;
        _global.onerror = handleUncaughtException;
      } else {
      ...
      function handleUncaughtException(err) {
        ...
        assert(msg.match(/\byep\b/), "got expected error"); // Restore Mocha's global error handler.

        if (process.browser) {
          _global.onerror = mochaHandler;
        } else {
          process.on("uncaughtException", mochaHandler);
        }

        done(); // Don't leak error to browser console
