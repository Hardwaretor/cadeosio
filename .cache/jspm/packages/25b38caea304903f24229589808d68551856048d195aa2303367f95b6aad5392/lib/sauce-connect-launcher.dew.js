...
import { dew as _indexDewDew } from "rimraf/index.dew.js";
import _os from "os";
import { dew as _indexDew2Dew } from "lodash/index.dew.js";
import { dew as _indexDew3Dew } from "async/index.dew.js";
import _https from "https";
import { dew as _indexDew4Dew } from "https-proxy-agent/index.dew.js";
import { dew as _indexDew5Dew } from "adm-zip/index.dew.js";
import _child_process from "child_process";
import _crypto from "crypto";
import { dew as _process_optionsDewDew } from "./process_options.dew.js";
import { dew as _packageJsonDewDew } from "../package.json.dew.js";
import { dew as _try_runDewDew } from "./try_run.dew.js";
...
export function dew() {
  ...
  var fs = _fs,
      ...
      async = _indexDew3Dew(),
      https = _https,
      HttpsProxyAgent = _indexDew4Dew(),
      AdmZip = _indexDew5Dew(),
      spawn = _child_process.spawn,
      exec = _child_process.exec,
      crypto = _crypto,
      processOptions = _process_optionsDewDew(),
      scDir = path.normalize((import.meta.url.startsWith('file:') ? decodeURI(import.meta.url.slice(0, import.meta.url.lastIndexOf('/')).slice(7 + (typeof process !== 'undefined' && process.platform === 'win32'))) : new URL(import.meta.url.slice(0, import.meta.url.lastIndexOf('/'))).pathname) + "/../sc"),
      exists = fs.existsSync || path.existsSync,
      currentTunnel,
      logger = console.log,
      cleanup_registered = false,
      sc_version = process.env.SAUCE_CONNECT_VERSION || _packageJsonDewDew().sauceConnectLauncher.scVersion,
      tunnelIdRegExp = /(?:Tunnel ID|Provisioned tunnel):\s*([a-z0-9]+)/i,
      portRegExp = /port\s*([0-9]+)/i,
      tryRun = _try_runDewDew(),
  ...
  function clean(callback) {
    async.series([killProcesses, function (next) {
      rimraf(scDir, next);
    }], callback);
  }
  ...
  function unpackArchive(archivefile, callback) {
    ...
    function done(err) {
      if (err) {
        return callback(new Error("Couldn't unpack archive: " + err.message));
  ...
  function setExecutePermissions(bin, callback) {
    if (os.type() === "Windows_NT") {
      ...
      fs.stat(bin, function (err, stat) {
        if (err) {
          return callback(new Error("Couldn't read sc permissions: " + err.message));
        }

        if (stat.mode.toString(8) !== "100755") {
          fs.chmod(bin, 0o755, function (err) {
            if (err) {
              return callback(new Error("Couldn't set permissions: " + err.message));
            }
  ...
  function fetchArchive(archiveName, archivefile, callback) {
    ...
    req.on("response", function (res) {
      ...
      res.pipe(fs.createWriteStream(archivefile)); // cleanup if the process gets interrupted.
  ...
  function fetchAndCheckArchive(archiveName, archivefile, checksum, callback) {
    return async.waterfall([async.apply(fetchArchive, archiveName, archivefile), async.apply(verifyChecksum, archivefile, checksum)], callback);
  }

  function unpackAndFixArchive(archivefile, bin, callback) {
    return async.waterfall([async.apply(unpackArchive, archivefile), async.apply(setExecutePermissions, bin)], callback);
  }
  ...
  function fetchAndUnpackArchive(versionDetails, options, callback) {
    ...
    async.waterfall([async.apply(fetchAndCheckArchive, archiveName, archivefile, versionDetails.checksum), async.apply(unpackAndFixArchive, archivefile, bin)], callback);
  }
  ...
  function readVersionsFile(versionsfile, cb) {
    var versions = _nodeRequire(versionsfile)["Sauce Connect"];
  ...
  function download(options, callback) {
    ...
    async.waterfall([async.apply(getVersion, options), function (versionDetails, next) {
  ...
  function connect(bin, options, callback) {
    ...
    readyfile = path.normalize((os.tmpdir ? os.tmpdir() : os.tmpDir()) + "/" + readyFileName);
    args.push("--readyfile", readyfile); // Only add -x when it wasn't specified
    ...
    logger("Starting sc with args: " + args.join(" ").replace(/-u\ [^\ ]+\ /, "-u XXXXXXXX ").replace(/-k\ [^\ ]+\ /, "-k XXXXXXXX ").replace(/[0-9a-f]{8}\-([0-9a-f]{4}\-){3}[0-9a-f]{12}/i, "XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXX"));
    var spawnOptions = {};
    ...
    child.stdout.on("data", function (data) {
      ...
      previousData = lines.pop(); // only process full lines
  ...
  function downloadAndRun(options, callback) {
    ...
    async.waterfall([async.apply(downloadWithRety, options), function (bin, next) {
      return run(bin, options, next);
    }], callback);
  }

  exports = downloadAndRun;
  exports.download = downloadWithRety;
  exports.kill = killProcesses;
  exports.clean = clean;
