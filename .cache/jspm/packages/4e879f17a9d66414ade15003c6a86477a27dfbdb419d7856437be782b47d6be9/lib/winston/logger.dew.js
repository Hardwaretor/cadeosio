...
export function dew() {
  ...
  const asyncForEach = _forEachDewDew();
  ...
  const isStream = _indexDew3Dew();

  const ExceptionHandler = _exceptionHandlerDewDew();

  const RejectionHandler = _rejectionHandlerDewDew();

  const LegacyTransportStream = _legacyDewDew();

  const Profiler = _profilerDewDew();

  const {
    warn
  } = _commonDewDew();

  const config = _indexDew4Dew();
  /**
